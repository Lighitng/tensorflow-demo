...
# 读取水稻数据
# apiUse {dictionary} data
# {
#     title: '水稻名',
#     data: dataFrame
# }
# apiSuccessExample {dictionary} datas
# [...data]
def read_excel_sync():
    # 打开文件
    ...
    for row in range(sheet.nrows - 2):
        ...
        for (index, obj) in enumerate(datas):
            ...
            obj['data'].append(([((datetime(*xldate_as_tuple(rowdata[0], 0)) - datetime(2015, 7, 25, 0, 0, 0)).total_seconds() / 60) % 1440
])   + rowdata[startCols[index]:startCols[index]+6])
    dataFrames = []
    # 构建dataFrame数据
